...
class ServerActivity : BaseActivity() {
    ...
    private var flowSelection = 0
    private var securitySelection = 0
    private var streamSecuritySelection = 0
    private var allowInSecureSelection = 0
    private var streamFingerprintSelection = 0
    private var networkSelection = 0
    private var headerTypeSelection = 0
    private var headerTypeTitleSelection = 0
    private var streamAlpnSelection = 0
        ...
        sp_network?.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(
                ...
                config?.getProxyOutbound()?.getTransportSettingDetails()?.let { transportDetails ->
                    headerTypeSelection = Utils.arrayFind(types, transportDetails[0])
    ...
    private fun bindingServer(config: ServerConfig): Boolean {
        ...
        if (streamSecurity >= 0) {
            streamSecuritySelection = streamSecurity
            ...
            (streamSetting.tlsSettings ?: streamSetting.realitySettings)?.let { tlsSetting ->
                ...
                tlsSetting.fingerprint?.let {
                    ...
                    streamFingerprintSelection = utlsIndex
                ...
                if (streamSetting.tlsSettings != null) {
                    ...
                    if (allowinsecure >= 0) {
                        allowInSecureSelection = allowinsecure
        ...
        if (network >= 0) {
            ...
            sp_network?.isEnabled = false
            sp_network?.isClickable = false
        }
        return true
    }
    ...
    private fun saveStreamSettings(streamSetting: V2rayConfig.OutboundBean.StreamSettingsBean) {
        val network = networkSelection ?: return
        val type = headerTypeSelection ?: return
        ...
        val allowInsecureField = allowInSecureSelection ?: return
        val streamSecurity = streamSecuritySelection ?: return
        val utlsIndex = streamFingerprintSelection ?: return
        val alpnIndex = streamAlpnSelection ?: return
        ...
        var sni = streamSetting.populateTransportSettings(
            ...
            headerType = if (type >= 0 ) transportTypes(networks[network])[type] else transportTypes(networks[network])[0],
            host = requestHost,
            path = path,
            seed = path,
            quicSecurity = requestHost,
            key = path,
            mode = if (type >= 0 ) transportTypes(networks[network])[type] else transportTypes(networks[network])[0],
            serviceName = path
        )
